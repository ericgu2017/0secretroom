...
We only use ga to understand user engagement data and we didn't and we never will enable per user tracking. It's perfectly safe to use and no user specific data will be sent or collect. All GA will do is a random userid(generated by GA lib randomly) created a new chatroom. stuff like that. 
